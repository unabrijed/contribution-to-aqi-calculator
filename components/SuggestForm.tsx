...
export default function SuggestForm() {
  ...
  if (state === "idle") return (
    <div className="bg-[var(--surface-1)] border border-[var(--border)] rounded-2xl p-5">
      ...
      <p className="text-xs text-ink-500 mb-4">Know a smoke source we haven&apos;t covered? Submit it and we&apos;ll research and add it. Or message username: unabrijed (twitter or instagram).</p>
      <button onClick={() => setState("open")} className="text-xs font-mono text-ember-400 hover:text-ember-300 transition-colors">
        + suggest a brand or category →
      </button>
    </div>
  );
  ...
  if (state === "done") return (
    <div className="bg-[var(--surface-1)] border border-[var(--border)] rounded-2xl p-5 flex items-center gap-3">
      ...
      <span className="text-sm text-ink-300">Suggestion received and we will review and add it. Thank you.</span>
    </div>
  );
  ...
  return (
    <div className="bg-[var(--surface-1)] border border-[var(--border)] rounded-2xl p-5">
      <div className="text-sm font-display font-medium text-ink-200 mb-1">Suggest a brand or category</div>
      <p className="text-xs text-ink-500 mb-5 leading-relaxed">Even a rough description helps. Know the tar/nicotine numbers? Even better. You can also message username: unabrijed (twitter or instagram).</p>
